...
**Domain:** `Platform / Billing` · **Suite:** `Finance Document Suite` · **Accent:** `#67e8f9`
...
**Llm Billing Engine** is a standalone, browser-based AI workflow app for **Platform / Billing**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.

**Core job:** LLM usage → metering and billing.
...
|---|---:|:---:|---|---|
| `llm_usage`<br>LLM usage | text | Yes | — | Affects input: LLM usage. |
...
| `currency`<br>currency | select | No | USD | Affects customization: currency. |
| `provider_prices`<br>provider prices | text | No | — | Affects customization: provider prices. |
| `markup`<br>markup | text | No | — | Affects customization: markup. |
| `user_plans`<br>user plans | text | No | — | Affects customization: user plans. |
| `quotas`<br>quotas | text | No | — | Affects customization: quotas. |
| `alert_thresholds`<br>alert thresholds | slider | No | 50 | Affects customization: alert thresholds. |
| `billing_cycle`<br>billing cycle | text | No | — | Affects customization: billing cycle. |
...
- **currency**: USD, EUR, GBP, EGP, AED, SAR, USDT
...
uvicorn app.main:app --host 127.0.0.1 --port 9133
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9133`. |
...
```bash
curl http://127.0.0.1:9133/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9133/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9133 uvicorn app.main:app --host 0.0.0.0 --port 9133
```
...
- `plugins/llm-billing-engine.json` for plugin metadata.
